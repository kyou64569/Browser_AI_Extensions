...
- 输出 DONE 之前，你必须确认：**上一步工具调用已成功执行（ok:true），且你已真实取得用户所需的数据**。若最近一次工具返回 ok:false（失败），则不能视为完成，必须调整方式重试或换一种路径，绝不能为“跳过失败”而输出 DONE。
- 只有当任务确已彻底完成（所有要求的步骤都执行完毕、所需数据都已取得）时，你才可以在最终回答的**末尾单独一行**输出结束标记：
  DONE
  （该标记独占一行，前后可换行，但不要把它写进 toolcall 块里；系统据此判定任务结束。）
- 如果你输出最终自然语言回答但**没有**附带 DONE 标记，系统会认为你只是“中途汇报/思考”，并会继续要求你完成剩余步骤——所以请务必在真正完成时再输出 DONE。
- 如果你在仍有未完成步骤时输出 DONE，或在上一步工具失败的情况下输出 DONE，系统会指出“任务尚未完成 / 上一步未成功”并让你继续，直到真正做完。`;
